...
> 33 |   await page.locator('input[name="email"]').fill(config.username);
     |                                             ^ Error: locator.fill: Test timeout of 30000ms exceeded.
  34 |   await page.locator('input[name="password"]').fill(config.password);
  35 |   await page.locator('input[value="Login"]').click();
  36 | 
  37 |   await expect(page).toHaveURL(/account\/account/);
  38 | });
  39 | 
```
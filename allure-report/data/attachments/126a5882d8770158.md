...
```
Error: locator.setChecked: Error: strict mode violation: locator('input[name="newsletter"]') resolved to 2 elements:
    1) <input value="1" type="radio" name="newsletter"/> aka getByRole('radio', { name: 'Yes' })
    2) <input value="0" type="radio" name="newsletter" checked="checked"/> aka getByRole('radio', { name: 'No' })

Call log:
  - waiting for locator('input[name="newsletter"]')
...
```ts
  ...
  23 |     this.newsletterCheckbox = page.locator('input[name="newsletter"]');
  24 |     this.privacyCheckbox = page.locator('input[name="agree"]');
  25 |     this.continueButton = page.locator('input[value="Continue"]');
  26 |   }
  27 | 
  28 |   async openRegisterPage(url: string) {
  29 |     await this.page.goto(url);
  30 |   }
  31 | 
  32 |   async enterFirstName(firstName: string) {
  33 |     await this.firstNameInput.fill(firstName);
  34 |   }
  35 | 
  36 |   async enterLastName(lastName: string) {
  37 |     await this.lastNameInput.fill(lastName);
  38 |   }
  39 | 
  40 |   async enterEmail(email: string) {
  41 |     await this.emailInput.fill(email);
  42 |   }
  43 | 
  44 |   async enterTelephone(telephone: string) {
  45 |     await this.telephoneInput.fill(telephone);
  46 |   }
  47 | 
  48 |   async enterPassword(password: string) {
  49 |     await this.passwordInput.fill(password);
  50 |   }
  51 | 
  52 |   async enterConfirmPassword(confirmPassword: string) {
  53 |     await this.confirmPasswordInput.fill(confirmPassword);
  54 |   }
  55 | 
  56 |   async selectNewsletter(option: boolean) {
> 57 |     await this.newsletterCheckbox.setChecked(option);
     |                                   ^ Error: locator.setChecked: Error: strict mode violation: locator('input[name="newsletter"]') resolved to 2 elements:
  58 |   }
  59 | 
  60 |   async selectPrivacyPolicy(option: boolean) {
  61 |     await this.privacyCheckbox.setChecked(option);
  62 |   }
  63 | 
  64 |   async clickContinue() {
  65 |     await this.continueButton.click();
  66 |   }
  67 | }
  68 | 
```